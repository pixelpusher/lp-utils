 ...
 * @param {String} gcode 
 */
export function cleanGCode(gcode) {
 ...
 * Make an infinite Array iterator from an iterable - return a function that takes step size as arg 
  * @param {Array} iterableArray Array to iterate through infinitely
  * @param {Integer} step Number of steps (indices) to advance backwards or forwards in array
...
export const makeloop = (iterableArray, step = 1) => {
    if (!Array.isArray(iterableArray)) {
        throw new TypeError("infiter: needs type array, got type " + (typeof iterableArray));
    }
    ...
    const iter = (function* (_step) {
        ...
        while (true) {
            ...
            const reset = yield iterableArray[pos];
            if (reset) pos = -1;
        }
    })(step);
 ...
 */
export const countto = (num) => numrange(1, num);
 ...
 */
export const repeat = async (num, func) => {
    for (let i = 0; i < num; i++) {
        try {
